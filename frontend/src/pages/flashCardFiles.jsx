    ...
    return (
      <div className="h-full flex flex-col items-center justify-center">
        <p className="text-xl mb-4">
          No flashcards found with this difficulty level.
        </p>
        <button
      ...
      <div className="w-full max-w-2xl">
        <div className="mb-4 flex justify-between items-center">
          <h2 className="text-2xl font-bold">
            Learning: <span className="capitalize">{selectedDifficulty}</span>{" "}
            Cards
          </h2>
          ...
          {files.map((file) => (
            ...
            >
              <span className="flex gap-1">
                <File className="text-sm text-gray-500" />
                {file.name}
              </span>
            </div>
          ))}
      ...
      <div className="w-2/3 p-6 overflow-y-auto">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            {flashcards
              .filter(
                (card) => !selectedFileId || card.fileId === selectedFileId,
              )
              .map((flashcardSet) => (
                <div
                  key={flashcardSet._id || flashcardSet.id}
                  className="border border-gray-200 rounded-lg p-4 hover:shadow-md"
                >
                  <div className="flex justify-between mb-2">
                    <span className="text-sm font-medium">
                      {flashcardSet.question}
                    </span>
                  ...
                  {(flashcardSet.flashcards || []).map((card, index) => (
                    <div
                      key={index}
                      className="mt-2 p-2 border-t border-gray-100"
                    >
                  ...
                  {(flashcardSet.tags || []).length > 0 && (
                    <div className="mt-2 pt-2 border-t border-gray-100">
                      {flashcardSet.tags.map((tag, index) => (
                        <span
                          key={index}
                          className="inline-block bg-gray-100 rounded-full px-2 py-1 text-xs mr-1"
                        >
                          {tag}
                        </span>
                      ))}
              ...
              <div className="w-16 h-16 mb-4 relative">
                ...
                <div className="w-12 h-16 bg-white border-2 border-gray-800 mx-auto rounded-md relative">
                  ...
                  <div className="absolute inset-2">
                    {level.icon === "battery-low" && (
                      <div className="h-1/3 bg-gray-800 absolute bottom-0 w-full"></div>
                    )}
                    {level.icon === "battery-medium" && (
                      <div className="h-2/3 bg-gray-800 absolute bottom-0 w-full"></div>
                    )}
                    {level.icon === "battery-high" && (
                      <div className="h-full bg-gray-800 absolute bottom-0 w-full"></div>
                    )}
