...
const TodoPage = () => {
  ...
  const checkUserTraits = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_API_URL}/api/todos/user/traits`,
        {
          headers: {
  ...
  const fetchTodos = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_API_URL}/api/todos`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const completeTask = async (todoId) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_API_URL}/api/todos/${todoId}/complete`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setError(`Failed to complete task. Please try again: ${err}`);
    }
  };
  ...
  const reassignTask = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_API_URL}/api/todos/${selectedTaskId}/reassign`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      setError(`Failed to reassign task. Please try again: ${err}`);
    }
  };
              ...
              <div className="space-y-6">
                {periods.map((period) => {
                  ...
                  return (
                    <div
                      key={period}
                      className={`rounded-lg border p-4 ${periodInfo.color}`}
                    >
                            ...
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                ...
                                <div className="flex items-center text-xs text-gray-400">
                                  ...
                                  />
                                  <span>
                                    Created at {formatDeadline(task.createdAt)}
                                  </span>
                                </div>
                              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-sm text-gray-500 mb-4">
              This task was not completed during its scheduled time. Please
